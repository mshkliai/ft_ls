...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/06 12:01:54 by mshkliai          #+#    #+#             */
/*   Updated: 2018/05/10 15:05:35 by mshkliai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	plus(char *format, t_print num, long long int n, char w)
{
	t_print	a;

	a.res = 0;
	if (ft_strchr(format, '+') && n >= 0)
	{
		a.j = demo_itoa(n, a.end2, '+');
...
static	int	minus(char *str, t_print num, long long int *s, char w)
{
	int	res;
	...
	*s = demo_itoa(*s, str, '-');
	...
	else if (num.j <= (*s - 1) && ft_strchr(str, '-'))
	{
...
static int	simple(t_print num, long long int s, char *str, char w)
{
	if (num.i < 0)
	{
...
int			conv_d(char *form, long long int n)
{
	...
	w = num.res;
	num.res = 0;
	if ((num.res = plus(form, num, n, w)
				+ minus(str, num, &n, w)) > 0)
		return (num.res);
	else
	{
		ft_atoi(str) == 0 && num.j == 0 ? n -= 1 : n + 0;
		num.res += simple(num, n, str, w);
	}
	return (num.res);
}
